...
namespace TfmrLib.FEM
{
    public class GetDPAxiElecProblem : GetDPProblem
    {
        ...
        protected override void WriteGetDPFile()
        {
            ...
            f.WriteLine();
            f.WriteLine($"eps0={Constants.eps_0};");
            f.WriteLine();
            f.WriteLine("Group{");
            ...
            // Need to define Sur_Neu_Elec if there are Neumann BCs
            f.Write("  Sur_Neu_Elec = Region[{");
            ...
            // Need to assign dn on Sur_Neu_Elec if there are Neumann BCs
            foreach (var bc in BoundaryConditions.OfType<NeumannBoundaryCondition>())
            {
                f.WriteLine($@"Function {{ dn[{bc.Name}] = {bc.Flux}; }}");
            }
            ...
            // {
            //     if (region.TryGetProperty("eps_r", out double eps_r))
            //     {
            //         f.WriteLine($@"Function {{ eps_r[Region[{{{region.Name}}}]] = {eps_r}; }}");
            //     }
            //     else
            //     {
            ...
            {
                f.WriteLine($"      {{ Region {bc.Name}; Value {bc.Potential}; }}");
            }
            f.WriteLine("    }");
            f.WriteLine("  }");
            ...
            f.WriteLine("Group{");
            // Vol_Elec needs to exclude the conductor boundaries
            f.WriteLine("   Vol_Elec += Region[ProblemDomain];");
            f.WriteLine("}");
            ...
            f.WriteLine("    eps_r, // relative permittivity (in Vol_Elec)");
            ...
            f.WriteLine("      Integral { [ eps_r[] * eps0 * Dof{d v} , {d v} ];");
            ...
            f.WriteLine("          Term { [ -eps0*eps_r[] * {d v} ]; In Vol_Elec; Jacobian Vol; }");
            f.WriteLine("        }");
            f.WriteLine("      }");
            ...
            // f.WriteLine("          Integral { [ eps0*eps_r[] / 2. * VirtualWork[{d v}] ];");
            ...
            // f.WriteLine("          Integral {  [ eps0*eps_r[] / 2. * SquNorm[{d v}] ];");
            // f.WriteLine("            In Vol_Elec; Jacobian Vol; Integration Int;");
            // f.WriteLine("          }");
            // f.WriteLine("        }");
            // f.WriteLine("      }");
